...
def _capture_metrics(
    ...
    if not (container_metrics_enabled or python_metrics_enabled):
        if logger:
            logger.debug("No metrics to capture, shutting down metrics capture thread")
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if logger:
                logger.error("Exception caught during capture of metrics, will cease capturing")
            return False  # terminate the thread safely without interrupting the main thread
...
def start_run_metrics_thread(
    ...
    if not container_metrics_enabled and not python_metrics_enabled:
        if logger:
            logger.debug(
                "No collectable metrics, skipping run metrics thread. Are we running in a container?"
            )
        return None, None
